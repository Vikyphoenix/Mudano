...
def test_db_connection(conn_string: str, schema_name: str):
    try:
        ...
        schema_check = engine.execute("SELECT count(schema_name) "
                                      "FROM information_schema.schemata "
                                      "WHERE schema_name = '" + schema_name + "';")
        if schema_check.first()[0] == 0:
            raise ('The schema ' + schema_name + 'does not exist in the provided database, Please create the tables '
                                                 'and schema in Postgres')
        else:
            pass
        ...
        print('Connected to the target postgres DB and ' + schema_name + ' schema fetch successful !!!')
